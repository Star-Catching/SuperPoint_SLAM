...
int SPmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        float bestDist = 256;
        int bestIdx = -1;
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)float SPmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
{
    ...
    return dist;
}
        {
            ...
            const float dist = DescriptorDistance(dMP,dKF);float SPmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
{
    float dist = (float)cv::norm(a, b, cv::NORM_L2);
